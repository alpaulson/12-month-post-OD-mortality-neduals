...
outpatient2011_all <- fread("data/2011/under65/outpatient_under65.csv")
...
rm(mbsf_cc_summary2014,mbsf_oth_cc_summary2014,cca2014,ccb2014) 
...
rm(mbsf_cc_summary2015,mbsf_oth_cc_summary2015,cca2015,ccb2015) 
...
rm(mbsf_cc_summary2016,mbsf_oth_cc_summary2016,cca2016,ccb2016) 

```

# IDENTIFY ODS

ICD-9 codes 2011-2015; ICD-10 codes 2015-2017 (CMS changed October 1, 2015)
...
medparOD2011_all <- medpar2011_all %>%
  filter_at(vars(DGNS_1_CD:DGNS_2_CD), any_vars(. %in% ODs_icd9))
...
outpatientOD2011_all <- outpatient2011_all %>% filter_at(vars(ICD_DGNS_CD1:ICD_DGNS_CD2),
...
outpatientOD2012_all <- outpatient2012_all %>% filter_at(vars(ICD_DGNS_CD1:ICD_DGNS_CD2),
                                                 any_vars(. %in% ODs_icd9))
...
outpatientOD2013_all <- outpatient2013_all %>% filter_at(vars(ICD_DGNS_CD1:ICD_DGNS_CD2),
                                                 any_vars(. %in% ODs_icd9))
...
outpatientOD2014_all <- outpatient2014_all %>% filter_at(vars(ICD_DGNS_CD1:ICD_DGNS_CD2),
                                                 any_vars(. %in% ODs_icd9))
...
which(colnames(medpar2015_all)=="DGNS_1_CD") # column 154

medpar2015_icd10 <- medpar2015_all[,c(154:155)]

medpar2015_icd10 <- unique(unlist(lapply(medpar2015_icd10, grep, pattern=ODs_icd10, value=TRUE)))

medparOD2015_all <- medpar2015_all %>%
  filter_at(vars(DGNS_1_CD:DGNS_2_CD), any_vars(. %in% c(medpar2015_icd10, ODs_icd9)))

paste0("There were ",n_distinct(medparOD2015_all$BENE_ID)," inpatient ODs in 2015.")

test <- subset(medparOD2015_all, DGNS_E_CD_CNT == 0) # 20% did not have E code


####### Outpatient #

which(colnames(outpatient2015_all)=="ICD_DGNS_CD2") # column 43

outpt2015_icd10 <- outpatient2015_all[,c(42:43)]
...
outpatientOD2015_all <- outpatient2015_all %>%
  filter_at(vars(ICD_DGNS_CD1:ICD_DGNS_CD2), any_vars(. %in% c(outpt2015_icd10, ODs_icd9)))
...
which(colnames(medpar2016_all)=="DGNS_1_CD") # column 154

medpar2016_icd10 <- medpar2016_all[,c(154:155)]

medpar2016_icd10 <- unique(unlist(lapply(medpar2016_icd10, grep, pattern=ODs_icd10, value=TRUE)))

medparOD2016_all <- medpar2016_all %>%
  filter_at(vars(DGNS_1_CD:DGNS_2_CD), any_vars(. %in% c(medpar2016_icd10)))

paste0("There were ",n_distinct(medparOD2016_all$BENE_ID)," inpatient ODs in 2016.")

test <- subset(medparOD2016_all, DGNS_E_CD_CNT == 0) # 40% did not have E code


####### Outpatient #

outpt2016_icd10 <- outpatient2016_all[,c(42:43)]
...
outpatientOD2016_all <- outpatient2016_all %>%
  filter_at(vars(ICD_DGNS_CD1:ICD_DGNS_CD2), any_vars(. %in% c(outpt2016_icd10)))
...
which(colnames(medpar2017_all)=="DGNS_1_CD") # column 154

medpar2017_icd10 <- medpar2017_all[,c(154:155)]

medpar2017_icd10 <- unique(unlist(lapply(medpar2017_icd10, grep, pattern=ODs_icd10, value=TRUE)))

medparOD2017_all <- medpar2017_all %>%
  filter_at(vars(DGNS_1_CD:DGNS_2_CD), any_vars(. %in% c(medpar2017_icd10)))

paste0("There were ",n_distinct(medparOD2017_all$BENE_ID)," inpatient ODs in 2017.")

test <- subset(medparOD2017_all, DGNS_E_CD_CNT == 0) # % did not have E code


####### Outpatient #

outpt2017_icd10 <- outpatient2017_all[,c(42:43)]
...
outpatientOD2017_all <- outpatient2017_all %>%
  filter_at(vars(ICD_DGNS_CD1:ICD_DGNS_CD2), any_vars(. %in% c(outpt2017_icd10)))

paste0("There were ",n_distinct(outpatientOD2017_all$BENE_ID)," outpatient ODs in 2017.")

```

Merge Medpar ODs to one df
...
rm(medpar2011_all,medpar2012_all) 

```

Merge Outpt ODs to one df
...
rm(outpatient2011_all,outpatient2012_all)
...
### Collapse to 1 record per OD

Single overdoses may result in more than one medpar or outpatient claim (or a medpar and outpatient claim) when benes are transferred to facilities. In this analysis, I made the following decisions:

1 - Records with the same inpatient admission dates: select latest discharge date
2 - Records with the same inpatient discharge: select earliest admission
3 - Records with the same outpt claim from dates: select latest discharge date
4 - Records with the same outpt claim thru dates: select either
5 - Benes who transferred from outpt to inpatient: select inpatient

The complete OD files for medpar and outpatient will have each event included, but the 'all OD dates' file will only have the unique events. This is so I can later capture all the diagnosis codes between facilities. 

```{r Simplify to 1 record per OD}

# Look at benes who have discharge date from outpatient the same as inpatient admission date & designate as one OD 

# Check for benes who transferred from outpatient to outpatient facility 

# Would have the same outpt claim from OR same outpt claim thru date

outpt_clm_from <- outptOD_allODs %>% 
  group_by(BENE_ID, CLM_FROM_DT) %>%
  filter(n() > 1) 

outpt_thru_dt <- outptOD_allODs %>% 
  group_by(BENE_ID, CLM_THRU_DT) %>%
  filter(n() > 1)

# Check for benes who transferred from inpt to inpt 

medpar_adm_dt <- medparOD_allODs %>% 
  group_by(BENE_ID,ADMSN_DT) %>% 
  filter(n() > 1) 

medpar_dschrg_dt <- medparOD_allODs %>% 
  group_by(BENE_ID, DSCHRG_DT) %>%
  filter(n() > 1)

# All benes who have OD records in both inpatient and outpatient files (captures transfers and those who have separate events in inpt/outpt settings)

allODdts_allbenes_complete$dupe_medpar_adm <- ifelse(allODdts_allbenes_complete$CLM_ID %in% medpar_adm_dt$MEDPAR_ID, 1, 0)
allODdts_allbenes_complete$dupe_medpar_dschrg  <- ifelse(allODdts_allbenes_complete$CLM_ID %in% medpar_dschrg_dt$MEDPAR_ID, 1, 0)

allODdts_allbenes_complete$dupe_outpt_clmfrom <- ifelse(allODdts_allbenes_complete$CLM_ID %in% outpt_clm_from$CLM_ID, 1, 0)
allODdts_allbenes_complete$dupe_outpt_clmthru <- ifelse(allODdts_allbenes_complete$CLM_ID %in% outpt_thru_dt$CLM_ID, 1, 0)

allODdts_allbenes_complete$outpt_to_inpt <- coalesce(allODdts_allbenes_complete$OP_CLM_THRU_DT, allODdts_allbenes_complete$IP_ADMSN_DT)

# Records with the same inpatient admission dates (select latest discharge date)

df3 <- subset(allODdts_allbenes_complete, dupe_medpar_adm == 1)
df4 <- df3 %>%
  group_by(BENE_ID, IP_ADMSN_DT) %>%
  filter(n()>1) %>%
  slice(which.max(IP_DSCHRG_DT)) 
df_delete1 <- df3 %>%
  filter(CLM_ID %notin% df4$CLM_ID) 

allODdts_allbenes_complete <- allODdts_allbenes_complete %>%
  filter(CLM_ID %notin% df_delete1$CLM_ID)

# Records with the same inpatient discharge (select earliest admission)

df5 <- subset(allODdts_allbenes_complete, dupe_medpar_dschrg == 1) # There are 3 benes with na for discharge dates. I am removing them - no way of accounting for history after discharge
df6 <- df5 %>% 
  group_by(BENE_ID, IP_DSCHRG_DT) %>%
  filter(n()>1) %>%
  filter(DSCHRG_DSTNTN_CD == 65 | DSCHRG_DSTNTN_CD == 6) %>%
  slice(which.min(IP_ADMSN_DT)) 
df_delete2 <- df5 %>%
  filter(CLM_ID %notin% df6$CLM_ID) 

allODdts_allbenes_complete <- allODdts_allbenes_complete %>%
  filter(CLM_ID %notin% df_delete2$CLM_ID & DSCHRG_DSTNTN_CD != 30)
  
# Records with the same outpt claim from dates (select latest discharge date)

df7 <- subset(allODdts_allbenes_complete, dupe_outpt_clmfrom == 1) # 1025 benes, 2224 clms, same dxs per event, slice  to 1 
df8 <- df7 %>% 
  group_by(BENE_ID, OP_CLM_FROM_DT) %>%
  filter(n()>1) %>%
  slice(which.max(OP_CLM_THRU_DT))
df_delete3 <- df7 %>%
  filter(CLM_ID %notin% df8$CLM_ID) 

allODdts_allbenes_complete <- allODdts_allbenes_complete %>%
  filter(CLM_ID %notin% df_delete3$CLM_ID)

# Records with the same outpt claim thru dates (select either)

df9 <- subset(allODdts_allbenes_complete, dupe_outpt_clmthru == 1)
df10 <- df9 %>%
  group_by(BENE_ID, OP_CLM_FROM_DT) %>%
  filter(n()>1) %>%
  slice(1) 
df_delete4 <- df9 %>%
  filter(CLM_ID %notin% df10$CLM_ID) 

allODdts_allbenes_complete <- allODdts_allbenes_complete %>%
  filter(CLM_ID %notin% df_delete4$CLM_ID)


######

# Check for benes who transferred from outpt to inpt

df11 <- allODdts_allbenes_complete %>% 
  group_by(BENE_ID, outpt_to_inpt) %>%
  filter(n() > 1)

n_distinct(df11$outpt_to_inpt)

df12 <- df11 %>%
  group_by(BENE_ID,outpt_to_inpt) %>%
  filter(outpt1 == 1)

n_distinct(df12$outpt_to_inpt)

allODdts_allbenes_complete <- allODdts_allbenes_complete %>%
  filter(CLM_ID %notin% df12$CLM_ID)

allODdts_allbenes_complete$INDEX_DT <- coalesce(allODdts_allbenes_complete$IP_DSCHRG_DT, allODdts_allbenes_complete$OP_CLM_THRU_DT)
...
```{r 2014-2016 medpar & outpt all ODs}

allODdts_allbenes_study.years <- subset(allODdts_allbenes_complete, INDEX_DT >= "2014-01-01" & INDEX_DT <= "2016-12-31")

paste0("A total of ",n_distinct(allODdts_allbenes_study.years$BENE_ID)," beneficiaries had at least one inpatient or outpatient OD claim between 2014-2016.")

```

### Prepare for dual coverage analysis
...
# subset to columns that will be used to determine dual status

allODdts_allbenes_study.years_subset <- allODdts_allbenes_study.years[,c("BENE_ID","CLM_ID","INDEX_DT")]

# 2014 ODs ####

ODs_2014 <- subset(allODdts_allbenes_study.years_subset, INDEX_DT < "2015-01-01")

paste0("In 2014, ",n_distinct(ODs_2014$BENE_ID)," beneficiaries were discharged after an OD in an inpatient or outpatient event.")

# 2015 ODs ####

ODs_2015 <- subset(allODdts_allbenes_study.years_subset, INDEX_DT >= "2015-01-01" & INDEX_DT < "2016-01-01")


paste0("In 2015, ",n_distinct(ODs_2015$BENE_ID)," beneficiaries were discharged after an OD in an inpatient or outpatient event.")

# 2016 ODs ####

ODs_2016 <- subset(allODdts_allbenes_study.years_subset, INDEX_DT >= "2016-01-01" & INDEX_DT < "2017-01-01")

paste0("In 2016, ",n_distinct(ODs_2016$BENE_ID)," beneficiaries were discharged after an OD in an inpatient or outpatient event.")
...
Benes needed full dual status/no HMO for 6 months prior to OD and 12 months following (or until death) so surrounding years are also included

2014 MBSF ODs 

```{r combine 2014 ODs-dual status & HMO}
...
```{r combine 2015 ODs-dual status & HMO}
...
```{r 2016 combine ODs-dual status & HMO}
...
# Merge each mbsf with the corresponding OD df (Bene ID, claim ID, index date + mbsf eligibility info)
...
```{r Pull death data from 2014-2017 mbsfs}
...
# deaths2014thru2017 <- readRDS("deaths2014thru2017.Rds")
...
Assigning benes who died full dual status & no HMO from their month of death to the end of study.  I am determining eligibility ultimately by summing eligibility within required months, so this will give benes who die and benes who live the same number of eligible months. This is 19 because they have 6 months prior, 12 months following, and during the month of the OD. Anyone who dies but did not fulfill the criteria before death will be excluded because the total number of eligible months will not be 19.
...
paste0("CHECK: The number of benes in the new dataframe (",n_distinct(ODs_2014thru2016$BENE_ID),") should be the same as the benes in the original df (", n_distinct(allODdts_allbenes_studycohort$BENE_ID),").")
...
Note: based on full dual + no HMO from 6 months prior to OD/12 subsequent months (or until death)
...
paste0("A total of ",n_distinct(ODs_2014thru2016_eligible$BENE_ID)," beneficiaries who fulfilled the full dual/no HMO eligibility requirement had ",n_distinct(ODs_2014thru2016_eligible$CLM_ID)," eligible claims  in an inpatient or outpatient facility between 2014-2016.")
...
# Filter the whole OD dataframe that contains all ODs from 2011-2017 for benes that satisfied the full dual criteria and fulfilled the other MBSF criteria in the year of the ODER 

allODdts_allbenes_studycohort_2014 <- allODdts_allbenes_complete %>% filter(CLM_ID %in% ODs_2014thru2016_eligible$CLM_ID & BENE_ID %in% mbsf2014_location$BENE_ID & INDEX_DT > "2013-12-31")

allODdts_allbenes_studycohort_2015 <- allODdts_allbenes_complete %>% filter(CLM_ID %in% ODs_2014thru2016_eligible$CLM_ID & BENE_ID %in% mbsf2015_location$BENE_ID & (INDEX_DT > "2014-12-31" & INDEX_DT < "2016-01-01"))

allODdts_allbenes_studycohort_2016 <- allODdts_allbenes_complete %>% filter(CLM_ID %in% ODs_2014thru2016_eligible$CLM_ID & BENE_ID %in% mbsf2016_location$BENE_ID & (INDEX_DT > "2015-12-31" & INDEX_DT < "2017-01-01"))

allODdts_studymbsfbenes <- rbind(allODdts_allbenes_studycohort_2014,allODdts_allbenes_studycohort_2015,allODdts_allbenes_studycohort_2016)
...
firstODdt_2014thru2016_all <- allODdts_studymbsfbenes %>%
  group_by(BENE_ID) %>%
  slice(which.min(INDEX_DT))

```

### Exclude benes who did not survive

Exclude benes who did not survive the index OD. Also: I am excluding here the benes who died on date of discharge but did not have a claim with death noted.

Finally, identifying the study cohort of benes who survived at least 1 day after discharge.  

```{r Benes with who died-index event or same day}

# Time to death ####

firstODdt_2014thru2016_diedindex <- subset(firstODdt_2014thru2016_all,DSCHRG_DSTNTN_CD == 20)

firstODdt_2014thru2016 <- firstODdt_2014thru2016_all %>% 
  filter(DSCHRG_DSTNTN_CD !=20) # Exclude benes who died in the ER

firstODdt_2014thru2016$BENE_DEATH_DT <- deaths2014thru2017$BENE_DEATH_DT[match(firstODdt_2014thru2016$BENE_ID,deaths2014thru2017$BENE_ID)]

firstODdt_2014thru2016$BENE_DEATH_DT <- as.Date(firstODdt_2014thru2016$BENE_DEATH_DT,"%d%b%Y")

firstODdt_2014thru2016$days2death <- difftime(firstODdt_2014thru2016$BENE_DEATH_DT,firstODdt_2014thru2016$INDEX_DT, units = "days")

firstODdt_2014thru2016$days2death <- as.numeric(firstODdt_2014thru2016$days2death)

summary(firstODdt_2014thru2016$days2death)

# Inspect days2death that are less than 0. 

days2death.lt0 <- filter(firstODdt_2014thru2016,days2death < 0)
...
firstODdt_2014thru2016_0 <- subset(firstODdt_2014thru2016, days2death >= 0 | is.na(firstODdt_2014thru2016$days2death))

days2death.0 <- filter(firstODdt_2014thru2016_0,days2death == 0)

paste0(n_distinct(firstODdt_2014thru2016_diedindex$BENE_ID)," beneficiaries were excluded because they died during the index event, ",n_distinct(days2death.lt0$BENE_ID)," beneficiary was excluded because the claim occurred 111 days after the death date and  ",n_distinct(days2death.0$BENE_ID)," beneficiaries were excluded because they died on the date of discharge from care.")

# All who survived at least a day

firstODdt_2014thru2016_gt0 <- subset(firstODdt_2014thru2016_0, days2death > 0| is.na(firstODdt_2014thru2016_0$days2death))

paste0(n_distinct(firstODdt_2014thru2016_gt0$BENE_ID)," beneficiaries survived at least one day after discharge.")

# Avg days to death

summary(firstODdt_2014thru2016_gt0$days2death)

paste0("The median number of days to death after OD is ",median(firstODdt_2014thru2016_gt0$days2death, na.rm = TRUE)," and the mean number of days to death is ",mean(firstODdt_2014thru2016_gt0$days2death, na.rm = TRUE),".")

# 12-month mortality (dead12)

firstODdt_2014thru2016_gt0$dead12 <- ifelse(firstODdt_2014thru2016_gt0$days2death < 366, 1, 0)
  
table(firstODdt_2014thru2016_gt0$dead12)

firstODdt_2014thru2016_gt0$dead12[is.na(firstODdt_2014thru2016_gt0$dead12)] <- 0

firstODdt_2014thru2016 <- firstODdt_2014thru2016_gt0 # final firstODdt file for study

paste0("There are ",n_distinct(firstODdt_2014thru2016$BENE_ID)," beneficiaries in this study cohort.")

firstODdt_2014thru2016$died_all <- ifelse(firstODdt_2014thru2016$days2death < 366, 1, 2)

firstODdt_2014thru2016$died_all[is.na(firstODdt_2014thru2016$died_all)] <- 0
...
# n_distinct(allODdts_studymbsfbenes$CLM_ID)
...
### NoOD full duals-2015

NE duals who did not OD and survived all year

```{r noOD neduals alive all year 2015}
...
# Add death date & format to date - whole df

mbsfOD$BENE_DEATH_DT <- deaths2014thru2017$BENE_DEATH_DT[match(mbsfOD$BENE_ID,deaths2014thru2017$BENE_ID)]
mbsf2015_noOD$BENE_DEATH_DT <- deaths2014thru2017$BENE_DEATH_DT[match(mbsf2015_noOD$BENE_ID,deaths2014thru2017$BENE_ID)]

mbsfOD$BENE_DEATH_DT <- as.Date(mbsfOD$BENE_DEATH_DT,"%d%b%Y")
mbsf2015_noOD$BENE_DEATH_DT <- as.Date(mbsf2015_noOD$BENE_DEATH_DT,"%d%b%Y")

# OD and no OD pop to master dataframe

mbsf_master <- rbind(mbsfOD, mbsf2015_noOD)
...
saveRDS(mbsf_master, file = "mbsf_master.Rds")
...
Filtering the outpt and medpar dfs for all claims for OD benes for HCU later (completing now while they are open rather than reopening later) 

Filtered files are saved within the chunks

```{r Outpt HCU for OD pop}

# firstODdt_2014thru2016 <- readRDS(file = "firstODdt_2014thru2016.Rds") 

# Ouptpt

outptHCU_ODpop_13 <- subset(outpatient2013_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
outptHCU_ODpop_14 <- subset(outpatient2014_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
outptHCU_ODpop_15 <- subset(outpatient2015_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
outptHCU_ODpop_16 <- subset(outpatient2016_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
outptHCU_ODpop_17 <- subset(outpatient2017_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)

# Remove V1 and inconsistent columns (not necessary for analysis)

outptHCU_ODpop_13$V1 <- NULL
outptHCU_ODpop_14$V1 <- NULL
outptHCU_ODpop_15$V1 <- NULL
outptHCU_ODpop_16$V1 <- NULL
outptHCU_ODpop_17$V1 <- NULL

outptHCU_ODpop_17$CLM_RSDL_PYMT_IND_CD <- NULL
outptHCU_ODpop_17$PRVDR_VLDTN_TYPE_CD <- NULL
outptHCU_ODpop_17$RR_BRD_EXCLSN_IND_SW <- NULL
outptHCU_ODpop_17$CLM_MODEL_REIMBRSMT_AMT <- NULL

outptHCU_ODpop <- rbind(outptHCU_ODpop_13,outptHCU_ODpop_14,outptHCU_ODpop_15,outptHCU_ODpop_16,outptHCU_ODpop_17)

paste0("A total of ",n_distinct(outptHCU_ODpop$BENE_ID)," beneficiaries had outpatient records between 2013-2017.")

saveRDS(outptHCU_ODpop, file = "outptHCU_ODpop.Rds")
```
...
medparHCU_ODpop_13 <- subset(medpar2013_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
medparHCU_ODpop_14 <- subset(medpar2014_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
medparHCU_ODpop_15 <- subset(medpar2015_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
medparHCU_ODpop_16 <- subset(medpar2016_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)
medparHCU_ODpop_17 <- subset(medpar2017_all, BENE_ID %in% firstODdt_2014thru2016$BENE_ID)

# Remove V1 and columns inconsistent between years (not necessary for analysis)

medparHCU_ODpop_13$V1 <- NULL
medparHCU_ODpop_14$V1 <- NULL
medparHCU_ODpop_15$V1 <- NULL
medparHCU_ODpop_16$V1 <- NULL
medparHCU_ODpop_17$V1 <- NULL

medparHCU_ODpop_16$ACO_ID_NUM <- NULL
medparHCU_ODpop_17$ACO_ID_NUM <- NULL

medparHCU_ODpop_16$RC_ALLOGENEIC_STEM_CELL_AMT <- NULL
medparHCU_ODpop_17$RC_ALLOGENEIC_STEM_CELL_AMT <- NULL

medparHCU_ODpop_16$ISLET_ADD_ON_PYMT_AMT <- NULL
medparHCU_ODpop_17$ISLET_ADD_ON_PYMT_AMT <- NULL

medparHCU_ODpop_17$CLM_IP_INITL_MS_DRG_CD <- NULL
medparHCU_ODpop_17$VAL_CD_Q1_PYMT_RDCTN_AMT <- NULL

medparHCU_ODpop <- rbind(medparHCU_ODpop_13,medparHCU_ODpop_14,medparHCU_ODpop_15,medparHCU_ODpop_16,medparHCU_ODpop_17)

paste0("A total of ",n_distinct(medparHCU_ODpop$BENE_ID)," beneficiaries had inpatient records between 2013-2017.")

saveRDS(medparHCU_ODpop, file = "medparHCU_ODpop.Rds")
...
Files are saved within the above chunks as well as here

```{r SAVE}

saveRDS(mbsfOD, file="mbsfOD.Rds")
saveRDS(mbsf2015_noOD, file="mbsf2015_noOD.Rds") 
saveRDS(mbsf_master, file = "mbsf_master.Rds") # All benes with original vars only
